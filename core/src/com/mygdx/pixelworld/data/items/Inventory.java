...
public class Inventory implements Disposable {
    ...
    public void load(String saveLine, Player player) {
        ...
        for (int i = 0; i < itemStrings.length; i++) {
            ...
            if (split.length < 2) {
                logger.warn("Item " + itemString + " not valid. Skipping.");
                continue;
            }
            switch (split[0]) {
                case "WEAPON":
                    if (split.length != 3) {
                        logger.warn("Weapon " + itemString + " not valid. Skipping.");
                        break;
                    }
                ...
                case "SIGIL":
                    if (split.length != 2) {
                        logger.warn("Sigil " + itemString + " not valid. Skipping.");
                        break;
                    }
                ...
                case "ARMOR":
                    if (split.length != 3) {
                        logger.warn("Armor " + itemString + " not valid. Skipping.");
                        break;
                    }
                    set(new Armor(ArmorType.valueOf(split[1]), Integer.parseInt(split[2])), i);
                    break;
                default:
                    logger.warn("Item " + itemString + " unknown. Skipping.");
                    break;
            }
        }
    }
