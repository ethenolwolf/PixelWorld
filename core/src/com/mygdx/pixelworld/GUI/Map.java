...
public class Map {
    ...
    public void update(Vector2 playerPos) {
        ...
        if (playerPos.x - offset.x <= sw * Costants.X_LIMIT_MIN) offset.add(Game.deltaTime * SPEED, 0);
        else if (playerPos.x - offset.x >= sw * Costants.X_LIMIT_MAX) offset.add(-Game.deltaTime * SPEED, 0);
        if (playerPos.y - offset.y <= sh * Costants.Y_LIMIT_MIN) offset.add(0, Game.deltaTime * SPEED);
        else if (playerPos.y - offset.y >= sh * Costants.Y_LIMIT_MAX) offset.add(0, -Game.deltaTime * SPEED);
